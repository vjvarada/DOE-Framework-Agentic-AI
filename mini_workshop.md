...
This is the step most people rush and then wonder why nothing works. Go slow here.
...
#### 3b — What `.env.example` looks like

When you open it, you'll see placeholder text like this:
...
#### 3c — Fill in your real values

Replace the placeholders with the actual values you saved in the "Before You Start" section:
...
Use a plain text editor — Notepad on Windows, TextEdit on Mac, or any code editor.  
**Do not add quotes around the values.** Just paste them in directly.

> **Why a `.env` file?** Credentials never live inside the code. If you share the script or push it to GitHub, your token stays safe in `.env`, which is listed in `.gitignore` and stays on your machine only.

#### 3d — Double-check before running

Before moving to Section 4, confirm:

- [ ] `.env` file exists in the project root (not `.env.example`)
- [ ] `TELEGRAM_TOKEN` is filled in (the long string from @BotFather)
- [ ] `TELEGRAM_CHAT_ID` is filled in (the number from @userinfobot)
- [ ] No quotes around the values
- [ ] No spaces around the `=` sign

> **Note on `ddgs` (the web search tool):** This one requires no API key and no signup. It works immediately after `pip install`. You don't need to configure anything for it.
